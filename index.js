...
const { username, password, mode, author, tags, rated, date, R18 ,display} = config
...
const STAR_URL = `https://www.pixiv.net/bookmark.php?rest=${rest}&order=desc`
...
const FOLLOW_URL = `https://www.pixiv.net/bookmark.php?type=user&rest=${rest}&p=`
  ...
  async download ({ id, name, author }) {
    ...
    try {
      ...
      const res = await axios({
        ...
        headers: {
          'User-Agent': USER_AGENT,
          'Referer': `https://www.pixiv.net/bookmark.php?rest=${rest}&order=date_d`,
          'Cookie': this.cookie
        }
      })
      const $ = cheerio.load(res.data)
